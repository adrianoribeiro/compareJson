...
func ExecParallel(jsonFile string, jsonFileBkp string) (bool, error) {
	...
	for jsonResult.More() {
		...
		go compareParallel(jsonResult, jsonHash, isFound)
		if !<-isFound {
			return false, nil
		}
	}
...
func compareParallel(jsonResult *json.Decoder, jsonHash map[string]int, isFound chan bool) {
	defer wg.Done()
